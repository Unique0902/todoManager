...
export default function EmotionJournalEditPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    axios.get(`/api/emotion-journals/${id}`).then(res => {
      setDate(res.data.date || '');
      setContent(res.data.content || '');
      if (MOOD_OPTIONS.includes(res.data.mood_tag)) {
        setMoodTag(res.data.mood_tag);
        setCustomMood('');
      } else if (res.data.mood_tag) {
        setMoodTag('직접입력');
        setCustomMood(res.data.mood_tag);
      } else {
        setMoodTag('');
        setCustomMood('');
      }
      setLinkedRoutineId(res.data.linked_routine_id || '');
    });
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await axios.patch(`/api/emotion-journals/${id}`, {
      date,
      content,
